...
def test_explicit_command_path_overrides_argv_derivation(tmp_path: Path) -> None:
    """When argv starts with a global flag, command_path can be set explicitly
    to avoid the leading-non-flag heuristic producing an empty path."""
    app = typer.Typer()
    ...
    assert (
        telemetry.run_instrumented(
            app, ["--verbose", "sync"], prog_name="t", path=db, command_path=["sync"]
        )
        == 0
    )
